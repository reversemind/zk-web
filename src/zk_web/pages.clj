...
(defpartial node-stat [stat]
  [:div.span4 [:table.table-striped.table-bordered.table [:tr [:h3 "Node Stat"]]
               (map (fn [kv]
                      [:tr [:td (first kv)]
                       [:td (last kv)]])
                 stat)]])
...
(defpartial create-modal [path]
  [:div#createModal.modal.hide.fade [:div.modal-header [:h4 "Create A Child"]]
   (form-to [:post "/create"]
     [:div.modal-body [:div.alert.alert-info "Create a child under: " [:strong path]]
     ...
     [:div.modal-footer [:button.btn.btn-danger "Create"]
      (space 1)
      [:button.btn.btn-success {:data-dismiss "modal"} "Cancel"]])
   ])

(defpartial edit-modal [path data]
  [:div#editModal.modal.hide.fade [:div.modal-header [:h4 "Edit Node Data"]]
   (form-to [:post "/edit"]
     [:div.modal-body [:div.alert.alert-info "Editing node: " [:strong path]]
      [:textarea.input.span7 {:type "text" :name "data" :rows 6} (bytes->str data)]
      [:input.span8 {:type "hidden" :name "path" :value path}]]
     [:div.modal-footer [:button.btn.btn-danger "Save"]
      (space 1)
      [:button.btn.btn-success {:data-dismiss "modal"} "Cancel"]
      ])])

(defpartial delete-modal [path children]
  [:div#deleteModal.modal.hide.fade [:div.modal-header [:h4 "Delete This Node"]]
   (form-to [:post "/delete"]
     [:input {:type "hidden" :name "path" :value path}]
     (let [child-num (count children)]
       (if (zero? child-num)
         [:div [:div.modal-body [:div.alert.alert-warn "Confirm to delete node: " [:strong path]]]
          [:div.modal-footer [:button.btn.btn-danger "Delete"]
           [:button.btn.btn-success {:data-dismiss "modal"} "Cancel"]]]
         [:div [:div.modal-body [:div.alert.alert-error "You can't delete a node with children."]]
          [:div.modal-footer [:button.btn.btn-success {:data-dismiss "modal"} "Cancel"]]]
         )))])

(defpartial rmr-modal [path]
  [:div#rmrModal.modal.hide.fade [:div.modal-header [:h4 "Delete This Node And It's Children"]]
   (form-to [:post "/rmr"]
     [:input {:type "hidden" :name "path" :value path}]
     [:div.modal-body [:div.alert.alert-error [:h4 "Danger!!"] "RMR will delete " [:strong path] " and all it's children!"]]
     [:div.modal-footer [:button.btn.btn-danger "I will perform RMR"]
      (space 1)
      [:button.btn.btn-success {:data-dismiss "modal"} "Cancel"]])
   ])
  ...
  (let [cookie (cookies/get :history )
    ...
    (layout
      ...
      [:form.well.span8 {:action "/init" :method "get"}
       [:div.span8 [:div.row [:div.span6 [:input.span6 {:type "text" :name "addr" :placeholder "Connect String: host[:port][/namespace]"}]]
                    [:div.span2 [:button.btn.btn-primary {:type "submit"} "Go"]]]]]
      )))
...
(defpage [:get "/login"] {:keys [msg target]}
  (layout
    [:div.span3.offset3 [:div.row (when-not (nil? msg) [:div.alert.alert-error [:h4 msg]])
                         (form-to [:post "/login"]
                           ...
                           [:div.form-actions [:button.btn.btn-primary {:type "submit"} "Login"]])]]))
